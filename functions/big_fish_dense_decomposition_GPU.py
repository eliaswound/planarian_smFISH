...
then use gaussian functions to correct a misdetection in these regions with
...
def decompose_dense(
    ...
    simulate a more realistic number of spots in these regions with optional
    GPU acceleration.
...
def get_dense_region(image, spots, voxel_size, spot_radius, beta=1, use_gpu=False):
    """Detect and filter dense and bright regions with optional GPU
    acceleration.
    ...
    connected_regions = _get_connected_region(image, threshold, use_gpu=use_gpu)
...
def _get_connected_region(image, threshold, use_gpu=False):
    """Find connected regions above a fixed threshold with optional GPU
    acceleration.
    ...
    # find connected components - use GPU if available
    if use_gpu and HAS_CUPY:
        ...
        # find connected components on CPU
        cc = label(mask)
