...
# Modified: Added CUDA/GPU support

"""
Functions to detect spots in 2-d and 3-d with GPU acceleration support.
"""
...
def _get_array_module(array):
    """Get the appropriate array module (CuPy or NumPy) based on array type."""
...
def _log_filter_gpu(image, log_kernel_size):
    """GPU-accelerated LoG filter using CuPy.
    
    This function replicates bigfish.stack.log_filter behavior on GPU.
    For now, we fall back to CPU implementation as CuPy's LoG implementation
    may differ. The GPU acceleration is primarily in maximum_filter and labeling.
    """
    if not HAS_CUPY:
        # Fallback to CPU
        return stack.log_filter(image, log_kernel_size)
    
    # For now, use CPU LoG filter as it's more reliable
    # GPU acceleration is applied in maximum_filter and labeling operations
    # which are the most computationally expensive parts
    return stack.log_filter(image, log_kernel_size)
...
def detect_spots(
    ...
    in a 2-d or 3-d image with optional GPU acceleration.
    ...
    use_gpu : bool or None
        Whether to use GPU acceleration. If None, automatically detects GPU
        availability and uses it if available.
...
def _detect_spots_from_images(
    ...
    #. A pixel which has the same value in the original and filtered images
       is a local maximum.
    #. We remove local peaks under a threshold.
    #. We keep only one pixel coordinate per spot.
    ...
    masks = []
    for image in images:
        # filter image - use GPU if available
        if use_gpu and HAS_CUPY:
            image_filtered = _log_filter_gpu(image, log_kernel_size)
            # Convert to numpy for compatibility with rest of pipeline
            image_filtered = _to_numpy(image_filtered)
        else:
        ...
        mask_local_max = local_maximum_detection(
            image_filtered, min_distance, use_gpu=use_gpu)
...
def local_maximum_detection(image, min_distance, use_gpu=False):
    """Compute a mask to keep only local maximum, in 2-d and 3-d with optional
    GPU acceleration.
    ...
    kernel_size = tuple(2 * min_distance + 1)
    ...
    if use_gpu and HAS_CUPY:
        ...
        # apply maximum filter on GPU
        image_filtered = cndi.maximum_filter(image_gpu, size=kernel_size)
        
        # we keep the pixels with the same value before and after the filtering
        mask_gpu = image_gpu == image_filtered
        
        # Convert back to numpy
        mask = _to_numpy(mask_gpu)
    else:
...
def spots_thresholding(
        ...
        remove_duplicate=True,
        use_gpu=False):
    """Filter detected spots and get coordinates of the remaining spots with
    ...
    if remove_duplicate:
        # Use GPU labeling if available
        if use_gpu and HAS_CUPY:
            # Convert to CuPy for labeling
            mask_gpu = cp.asarray(mask)
            cc_gpu = cndi.label(mask_gpu)[0]
            cc = _to_numpy(cc_gpu)
            
            # Get centroids using regionprops (CPU operation)
        ...
        if len(spots) > 0:
            if image.ndim == 3:
                mask[spots[:, 0].astype(int), spots[:, 1].astype(int), spots[:, 2].astype(int)] = True
            else:
                mask[spots[:, 0].astype(int), spots[:, 1].astype(int)] = True
...
def get_elbow_values(
    ...
    threshold to detect spots with optional GPU acceleration.
    ...
    for image in images:
        ...
        # find local maximum
        mask_local_max = local_maximum_detection(
            image_filtered, minimum_distance, use_gpu=use_gpu)
        masks.append(mask_local_max)
